...
import { Board, Player, Tetromino, PieceKey } from '../types';
import { BOARD_WIDTH, BOARD_HEIGHT, PIECE_KEYS, TETROMINOS } from '../constants';
import { isColliding, rotate } from '../gameLogic';

const getAggregateHeight = (board: Board): number => {
    let totalHeight = 0;
...
const getHoles = (board: Board): number => {
    let holes = 0;
    for (let x = 0; x < BOARD_WIDTH; x++) {
...
const getBumpiness = (board: Board): number => {
    let bumpiness = 0;
    const heights = [];
    for (let x = 0; x < BOARD_WIDTH; x++) {
        let height = 0;
...
const getLinesCleared = (board: Board): number => {
...
const evaluateBoard = (board: Board): number => {
    ...
    // Weights prioritize avoiding holes and high stacks. Clearing lines is very good.
    return (lines * 10) - (height * 0.5) - (holes * 4) - (bumpiness * 0.2);
};

export const getPieceSuggestions = (board: Board): Tetromino[] => {
        ...
        for (let r = 0; r < 4; r++) {
            ...
            for (let x = -2; x < BOARD_WIDTH; x++) {
                const tempPlayer: Player = {
                    pos: { x, y: 0 },
                    tetromino: { ...piece, shape: currentPieceShape },
                    collided: false,
                };
                ...
                const tempBoard: Board = board.map(row => row.map(cell => [...cell]));
    ...
    if (isBoardEmpty) {
        ...
        // Sort from best score (highest) to worst score (lowest).
        // This is "least inefficient" to "most inefficient".
        pieceScores.sort((a, b) => b.score - a.score);
    }

    return pieceScores.map(item => TETROMINOS[item.pieceKey]);
};
